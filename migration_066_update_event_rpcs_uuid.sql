-- Migration 066: Update event RPCs for Clerk UUIDs
-- Date: 2026-02-06
-- Description: Use current_user_uuid() and UUID comparisons in event RPCs
...
BEGIN
    ...
    WHERE user_id = v_user_uuid AND status = 'active'
    LIMIT 1;
...
BEGIN
    ...
    IF v_event.created_by_id <> v_user_uuid THEN
    ...
    LOOP
        IF (v_item->>'id') = p_item_id THEN
            ...
            IF v_claimed_by IS NULL OR v_claimed_by = '' THEN
                ...
                v_item := jsonb_set(v_item, '{claimed_by_id}', to_jsonb(v_user_uuid::text), true);
                v_item := jsonb_set(v_item, '{claimed_by_name}', to_jsonb(COALESCE(v_profile.display_name, 'Neighbor')), true);
            ELSIF v_claimed_by = v_user_uuid::text THEN
...
BEGIN
    ...
    FROM public.user_profiles WHERE id = v_user_uuid;
...
BEGIN
    ...
    LOOP
        IF v_claim->>'id' = p_claim_id THEN
            IF v_claim->>'user_id' != v_user_uuid::text AND v_event.created_by_id != v_user_uuid THEN
...
BEGIN
    ...
    IF v_event.created_by_id <> v_user_uuid THEN
        IF NOT EXISTS (SELECT 1 FROM public.speakers WHERE id::text = v_speaker_id AND created_by_id = v_user_uuid) THEN
...
-- Invite speaker by email (uuid-based)
...
BEGIN
    ...
    WHERE created_by_id = v_user_uuid
